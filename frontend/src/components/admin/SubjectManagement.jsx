...
function SubjectManagement() {
  ...
  const validateForm = () => {
    const errors = { MaKhoa: '', TenMonHoc: '', SoTinChi: '' };
    let isValid = true;
    
    const tenMonHocFormatted = formatTitleCase(formData.TenMonHoc.trim());

    if (!formData.MaKhoa) {
      errors.MaKhoa = 'Vui lòng chọn khoa'; isValid = false;
    }

    if (!tenMonHocFormatted) {
      errors.TenMonHoc = 'Vui lòng nhập tên môn học'; isValid = false;
    } else if (tenMonHocFormatted.length < 3) {
      errors.TenMonHoc = 'Tên môn học phải có ít nhất 3 ký tự'; isValid = false;
    } else if (tenMonHocFormatted.length > 50) { 
      errors.TenMonHoc = 'Tên môn học không được vượt quá 50 ký tự'; isValid = false;
    } else if (/\d/.test(tenMonHocFormatted)) {
      errors.TenMonHoc = 'Tên môn học không được chứa số'; isValid = false;
    } else if (/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(tenMonHocFormatted)) {
      errors.TenMonHoc = 'Tên môn học không được chứa ký tự đặc biệt'; isValid = false;
    } else if (/\s{2,}/.test(tenMonHocFormatted)) {
      errors.TenMonHoc = 'Tên môn học không được chứa nhiều khoảng trắng liên tiếp'; isValid = false;
    } else {
      ...
      if (duplicateSubject) {
        errors.TenMonHoc = 'Tên môn học này đã tồn tại!'; isValid = false;
      }
    }

    if (!formData.SoTinChi) {
      errors.SoTinChi = 'Vui lòng nhập số tín chỉ'; isValid = false;
    } else {
      const credits = Number(formData.SoTinChi);
      if (isNaN(credits)) { errors.SoTinChi = 'Số tín chỉ phải là số'; isValid = false; } 
      else if (!Number.isInteger(credits)) { errors.SoTinChi = 'Số tín chỉ phải là số nguyên'; isValid = false; } 
      else if (credits < 1) { errors.SoTinChi = 'Số tín chỉ phải lớn hơn 0'; isValid = false; } 
      else if (credits > 9) { errors.SoTinChi = 'Số tín chỉ không được vượt quá 9'; isValid = false; }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // SỬ DỤNG API_BASE TẠI ĐÂY
      if (editingSubject) {
        await axios.put(`${API_BASE}/subjects/${editingSubject.MaMonHoc}`, dataToSubmit);
        ...
        await axios.post(`${API_BASE}/subjects`, { ...dataToSubmit, MaMonHoc: resCode.data.MaMonHoc });
  ...
  const handleCloseDetailModal = () => {
    ...
    setSubjectClasses([]);
    setSubjectTeachers([]);
    setSubjectGradeStats({ totalGrades: 0, average: 0, excellent: 0, good: 0, averageGrade: 0, fail: 0 });
    setDetailTab('classes');
  };
  ...
  return (
    <div className="space-y-8 bg-gray-50/50 p-4 rounded-2xl">
      ...
      <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-6">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            ...
            />
            {searchTerm && (
              <button onClick={handleClearSearch} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors">
                <X className="w-5 h-5" />
              </button>
            )}
          </div>
          <div className="flex gap-3">
            ...
            >
              ...
              {activeFilterCount > 0 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold rounded-full w-5 h-5 flex items-center justify-center">{activeFilterCount}</span>}
            </motion.button>
            {hasActiveFilters && (
              <motion.button
                whileHover={{ scale: 1.02 }}
              ...
              >
                <XCircle className="w-5 h-5" /> Xóa lọc
              </motion.button>
            )}
          </div>
        </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1">
                {[...Array(totalPages)].map((_, i) => (
                  <button key={i + 1} onClick={() => setCurrentPage(i + 1)} className={`w-8 h-8 rounded-lg text-sm font-medium transition-all ${currentPage === i + 1 ? 'bg-orange-500 text-white border-orange-500 shadow-md shadow-orange-500/20' : 'text-gray-600 hover:bg-gray-100 border border-transparent'}`}>{i + 1}</button>
                ))}
              </div>
              <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="p-2 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition-all"><ChevronRight className="w-5 h-5" /></button>
            </div>
          </div>
        )}
      </div>
      ...
      {showModal && (
        <ModalPortal>
          <div className="fixed inset-0 flex items-center justify-center z-50 p-4 bg-black/40 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            <div className="bg-gradient-to-r from-orange-500 to-orange-600 px-6 py-5 flex justify-between items-center flex-shrink-0">
              <div className="text-white">
                <h3 className="text-xl font-bold flex items-center gap-2"><BookOpen className="w-5 h-5" /> {editingSubject ? 'Cập nhật môn học' : 'Thêm môn học mới'}</h3>
                <p className="text-orange-100 text-sm mt-0.5">{editingSubject ? 'Chỉnh sửa thông tin môn học' : 'Tạo môn học theo khoa chuyên môn'}</p>
              </div>
              <button onClick={handleCloseModal} className="p-2 hover:bg-white/20 rounded-lg text-white"><X className="w-5 h-5" /></button>
            </div>

            <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto" onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleSubmit(e); } }}>
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Khoa</label>
                <select value={formData.MaKhoa} onChange={handleKhoaChange} className={`w-full px-4 py-3 bg-gray-50 border-2 rounded-xl focus:outline-none transition-colors text-gray-700 ${formErrors.MaKhoa ? 'border-red-500 focus:border-red-500' : 'border-gray-200 focus:border-orange-500'}`}>
                  <option value="">Chọn khoa</option>
                  {faculties.map((faculty) => ( <option key={faculty.MaKhoa} value={faculty.MaKhoa}>{faculty.TenKhoa}</option> ))}
                </select>
              ...
              <div>
                ...
                <input type="text" value={formData.TenMonHoc} onChange={(e) => { setFormData({ ...formData, TenMonHoc: e.target.value }); if (formErrors.TenMonHoc) setFormErrors({ ...formErrors, TenMonHoc: '' }); }} onBlur={(e) => { const formatted = formatTitleCase(e.target.value.trim()); setFormData(prev => ({ ...prev, TenMonHoc: formatted })); }} maxLength={50} className={`w-full px-4 py-3 bg-gray-50 border-2 rounded-xl focus:outline-none transition-colors ${formErrors.TenMonHoc ? 'border-red-500 focus:border-red-500' : 'border-gray-200 focus:border-orange-500'}`} />
              ...
              <div>
                ...
                <input type="number" min="1" max="10" value={formData.SoTinChi} onChange={(e) => { setFormData({ ...formData, SoTinChi: e.target.value }); if (formErrors.SoTinChi) setFormErrors({ ...formErrors, SoTinChi: '' }); }} className={`w-full px-4 py-3 bg-gray-50 border-2 rounded-xl focus:outline-none transition-colors ${formErrors.SoTinChi ? 'border-red-500 focus:border-red-500' : 'border-gray-200 focus:border-orange-500'}`} />
                {formErrors.SoTinChi && <p className="text-red-500 text-xs mt-1">{formErrors.SoTinChi}</p>}
              </div>
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={handleCloseModal} className="flex-1 py-3 bg-gray-100 text-gray-700 font-semibold rounded-xl hover:bg-gray-200">Hủy</button>
                <button type="submit" disabled={isSubmitting} className="flex-1 py-3 bg-gradient-to-r from-orange-500 to-orange-600 text-white font-semibold rounded-xl shadow-lg disabled:opacity-60">{isSubmitting ? 'Đang lưu...' : (editingSubject ? 'Lưu thay đổi' : 'Thêm môn học')}</button>
              </div>
            </form>
          </motion.div>
        </div>
      ...
      {showDetailModal && selectedSubject && (
        <ModalPortal>
          <div className="fixed inset-0 flex items-center justify-center z-50 p-4 backdrop-blur-sm bg-black/40">
            <motion.div initial={{ opacity: 0, scale: 0.92, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.92, y: 20 }} transition={{ type: 'spring', stiffness: 300, damping: 30 }} className="bg-white rounded-3xl w-full max-w-5xl max-h-[92vh] overflow-hidden shadow-2xl flex flex-col">
            <div className="bg-gradient-to-r from-orange-500 to-orange-600 px-8 py-6 flex-shrink-0">
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    <div className="bg-white/20 rounded-xl p-2"><BookOpen className="w-6 h-6 text-white" /></div>
                ...
                <motion.button whileHover={{ scale: 1.1, rotate: 90 }} whileTap={{ scale: 0.9 }} onClick={handleCloseDetailModal} className="bg-white/20 hover:bg-white/30 rounded-xl p-2 transition-colors"><X className="w-5 h-5 text-white" /></motion.button>
              </div>

              <div className="flex gap-1 mt-5">
                {[ { id: 'classes', label: 'Lớp học', icon: BookOpen }, { id: 'teachers', label: 'Giảng viên', icon: Users }, { id: 'stats', label: 'Thống kê điểm', icon: BarChart3 } ].map(tab => {
                  const Icon = tab.icon;
                  return (
                    <button key={tab.id} onClick={() => setDetailTab(tab.id)} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all ${detailTab === tab.id ? 'bg-white text-orange-600 shadow-md' : 'text-white/70 hover:text-white hover:bg-white/10'}`}><Icon className="w-4 h-4" /> {tab.label}</button>
                  );
                })}
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {detailTab === 'classes' && (
                <div>
                  <h4 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-4">Danh sách lớp học ({subjectClasses.length})</h4>
                      ...
                      <table className="w-full">
                        ...
                        <tbody>
                          {subjectClasses.map((cls, index) => (
                            <motion.tr key={index} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: index * 0.03 }} className="border-t border-gray-50 hover:bg-orange-50/40 transition-colors">
                              ...
                              <td className="py-3.5 px-5"><span className="bg-orange-100 text-orange-700 px-2.5 py-1 rounded-md font-bold text-sm">{cls.SoSinhVien || 0} SV</span></td>
                            </motion.tr>
                          ))}
                        </tbody>
                      </table>
              ...
              {detailTab === 'teachers' && (
                <div>
                  <h4 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-4">Giảng viên dạy môn ({subjectTeachers.length})</h4>
                  {subjectTeachers.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {subjectTeachers.map((teacher, index) => (
                        <motion.div key={index} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.06 }} className="flex items-center gap-3 bg-gray-50 rounded-xl p-4 border border-gray-100 hover:border-orange-200 hover:bg-orange-50/30 transition-all">
                          <div className="bg-orange-100 rounded-xl p-2 flex-shrink-0"><UserCheck className="w-5 h-5 text-orange-600" /></div>
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {[
                          ...
                          return (
                            <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} className={`rounded-2xl border-2 p-5 ${item.bg} ${item.border}`}>
                              <div className={`rounded-xl p-2 w-fit mb-3 ${item.iconBg}`}><Icon className={`w-5 h-5 ${item.text}`} /></div>
                        ...
                        {[
                          ...
                          return (
                            <div key={i} className="flex items-center gap-3 mb-3 last:mb-0">
                              ...
                              <div className="flex-1 bg-gray-200 rounded-full h-2.5 overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.7, delay: i * 0.1 }} className={`h-full rounded-full ${bar.color}`} /></div>
      ...
      <ConfirmDialog show={deleteModal.show} message={`Bạn có chắc chắn muốn xóa vĩnh viễn "${deleteModal.subject?.TenMonHoc}" "${deleteModal.subject?.MaMonHoc}"? Hành động này không thể hoàn tác.`} onConfirm={handleDeleteConfirm} onCancel={() => setDeleteModal({ show: false, subject: null })} title="Xác nhận xóa môn học" />
      <Toast show={toast.show} message={toast.message} type={toast.type} onClose={() => setToast({ ...toast, show: false })} />
      <SuccessDialog show={successDialog.show} message={successDialog.message} onClose={() => setSuccessDialog({ show: false, message: '' })} title="Thành công" />
      <ErrorDialog show={errorDialog.show} message={errorDialog.message} onClose={() => setErrorDialog({ show: false, message: '' })} title="Lỗi" />
    </div>
  );
}
