...
function TeacherManagement() {
  ...
  const handleKhoaChange = async (e) => {
    ...
    try {
      const res = await axios.get(`/api/teachers/next-code/${khoaId}`);
    ...
    try {
      if (editingTeacher) {
        setConfirmDialog({
          ...
          onConfirm: async () => {
            try {
              await axios.put(`/api/teachers/${editingTeacher.MaGiangVien}`, formData);
        ...
        const khoaId = selectedFaculty?.ID ?? selectedFaculty?.id ?? '';
        const resCode = await axios.get(`/api/teachers/next-code/${khoaId}`);
        const newMaGV = resCode.data.MaGiangVien;
        await axios.post('/api/teachers', { ...formData, MaGiangVien: newMaGV });
  ...
  const handleViewDetails = async (teacher) => {
    ...
    try {
      const [detailsRes, scheduleRes, loadRes] = await Promise.all([
        axios.get(`/api/teachers/${teacher.MaGiangVien}/details`),
        axios.get(`/api/teachers/${teacher.MaGiangVien}/teaching-schedule`),
        axios.get(`/api/teachers/${teacher.MaGiangVien}/teaching-load`)
      ]);
