    ...
    const fetchOverviewData = async () => {
      ...
      try {
        ...
        const [coursesRes, schedulesRes] = await Promise.all([
          axios.get(`/api/course-sections/teacher/${user.username}`),
          axios.get(`/api/teachers/${user.username}/teaching-schedule`)
        ]);
        ...
        const studentPromises = courses.map(c => 
          axios.get(`/api/course-sections/${c.MaLopHocPhan}/students`).catch(() => ({ data: [] }))
        );
